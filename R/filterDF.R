# DLM Filter using discount factors for transition covariance matrix -----------
filterDF <- function(y, mod, DF) {
    ...
    for(n in 1:N){
        ...
        # specify W using discount factor 
        W[, , n] <- GG %*% C[, , n] %*% t(GG) * ((1-DF)/DF)
        Wsvd <- La.svd(W[, , n] , nu = 0)
        ...
        #Qinv <- solve(Q[, , n])
        Qinv <- inverse_smw(V, FF, R[, , n], t(FF))
        m[, n+1] <- a[, n] + R[, , n] %*% t(FF) %*% Qinv %*% 
            as.matrix((y[n, ] - f[, n]), nrow = P, ncol = 1)
